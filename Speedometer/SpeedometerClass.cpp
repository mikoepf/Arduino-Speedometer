...
if(GetInput(ROTBUT))
{
  ...
  { // Iterates back to the startmenue (0)
    Profiles::count = Profiles::pn; // Resets the shown profile on the LCD to the current profile
...
const void UserInput::LCDprint(LiquidCrystal_I2C &lcd,Profile &ProfileCurrent )
{
  ...
  switch(menu_count)  // The menues shown on the LCD correspont to the currently selected menue (in the NavigateMenu-Method)
  {
    case STARTMENU: 
{ 
  ...
  lcd.print("Prof:"); 
  lcd.setCursor (6,0);
  lcd.print(ProfileCurrent.profile_name);  
  lcd.setCursor (0,1);
  ...
  lcd.print("Prof:"); 
  lcd.setCursor (6,0);
  lcd.print(Profiles::profile_names[Profiles::count]);  
  lcd.setCursor (0,1);
  lcd.print(Profiles::min_rpms[Profiles::count]);  
  lcd.setCursor (4,1);
  lcd.print(Profiles::max_rpms[Profiles::count]);  
  lcd.setCursor (8,1);
...
///////////////////////////////////// Checks and stores the current state of rotary encoder signal befor the Loop beginn.////////////////////////////////
...
bool UserInput::GetInput(const unsigned short int i)
{
  ...
  if (!digitalRead(i) && !pin_state[i] && (millis()-button_delay)>BUTTONDELAY) // If the previous and the current state of the rot_pin1 are different, 
                                            // that means a Pulse has occurred (rising flank)
  {
  pin_state[i] = 1;
  result = true;
  button_delay = millis();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int Copy::eeAddress=0;  // EEPROM address to store values
/////////////////////////////////////// Copy Profile 1/////////////////////////////////////////
// Copies the parameters of the currently selected profile (from the Profiles.cpp) to the referenced Profiel (usually current profile)
...
/////////////////////////////////////// Copy Profile 2/////////////////////////////////////////
// Copies the parameters of the profile with the "count"- number (from the Profiles.cpp) to the referenced Profiel (usually current profile)
...
/////////////////////////////////////// Copy Profile 3/////////////////////////////////////////
// Copies the parameters of the profile with the "count"- number (from the Profiles.cpp) to the referenced Profiel (usually current profile). Additionally a profile name can be assigned manually
...
{ // Since EEPROM can't store complex objects/classe like Strings, the String text has to be converted to char arrays.
struct EEProfile
{
  char profile_name[12];
  unsigned short int min_rpm;
  unsigned short int max_rpm;
  char filter_name[4];
  unsigned short int sample_size;
} EEPROF;
// Copying the values of the reference profile to the EEProfile struct and converting the String text to char arrays.
...
EEPROM.put(eeAddress, EEPROF);  // Saving the EEProfile struct in the EEPROM at the EEPROM address "eeAddress"
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Copy::LoadFromEEPROM(Profile & profile) // Loads the stored EEPROM parameter values to the referenced profile.
{
struct EEProfile
{
  char profile_name[12];
  ...
  if(int i = EEPROM.read(eeAddress) == 255) // Checks whether something is stored in the EEPROM at the eeAddress-address => If the memory is empty, 255 is returned.
  {
  ...
  {
    EEPROM.get( eeAddress, EEPROF );  // retreiving the stored values from the EEPROM and stroring them into the EEPROF Struct.
    // Since the EEPROF can only store char arrays, these have to be transformed into String text before copying the parameter values into the referenced profile (usually current profile)
